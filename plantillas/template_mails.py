...
class ReporteManager:
    ...
    def _generar_html_reporte_por_jefe(self, nombre_jefe, empleados_data):
        ...
        incidencias_map = {}
        if hasattr(self, 'incidencias_df') and not self.incidencias_df.empty:
            incidencias_agrupadas = self.incidencias_df.groupby('rut_empleado')
        
        for rut, group in incidencias_agrupadas:
            # Crear un string consolidado para cada empleado: "Tipo (Inicio - Fin); Tipo2 (Inicio2 - Fin2)"
            incidencias_list = []
            for _, row in group.iterrows():
                incidencia_str = f"{row['Tipo_Permiso_Formateado']} ({row['fecha_inicio_formato']} - {row['fecha_fin_formato']})"
                incidencias_list.append(incidencia_str)
            
            # Unir todas las incidencias con un salto de línea HTML (<br>)
            incidencias_map[rut] = "<br>".join(incidencias_list)
        ...
        for emp in empleados_data:
            rut_empleado = emp.get('rut') # Asumo que el RUT está disponible en 'empleados_data' con la clave 'rut'
        
        # 💡 MODIFICACIÓN 2: Obtener incidencias del mapa. Si no existen, asignar un mensaje.
            incidencias_html = incidencias_map.get(rut_empleado, '—') 
        
        # Si tiene incidencias, marcarlas con un estilo.
            if incidencias_html != '—':
                incidencias_html = f'<span class="alerta-incidencia"></span><br>{incidencias_html}'
            ...
            html += f"""
                            <tr class="{clase_fila}">
                                ...
                                <td>{incidencias_html}</td> 
                            </tr>
        """
    
    # --- FOOTER Y CIERRE DEL HTML (Sin cambios) ---
        html += """
                    </tbody>
                </table>
